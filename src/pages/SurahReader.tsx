...
import { ChevronLeft, ChevronRight, BookOpen, Bookmark, Settings, Play, Pause, SkipBack, SkipForward, Volume2, BookMarked, Eye } from "lucide-react";
...
const SurahReader = () => {
  ...
  const [activeTab, setActiveTab] = useState<"reading" | "arabic" | "tafsir">("reading");
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-3 sm:px-4 py-4 sm:py-6 max-w-4xl">
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="flex items-center gap-1 sm:gap-2">
                <Button variant="ghost" size="icon" onClick={previousVerse} disabled={currentVerse <= 1} className="w-8 h-8 sm:w-9 sm:h-9">
                  <SkipBack className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                </Button>
                <Button variant="spiritual" size="icon" className="w-10 h-10 sm:w-12 sm:h-12 rounded-full" onClick={togglePlay}>
                  {isPlaying ? <Pause className="w-4 h-4 sm:w-5 sm:h-5" /> : <Play className="w-4 h-4 sm:w-5 sm:h-5 ml-0.5" />}
                </Button>
                <Button variant="ghost" size="icon" onClick={nextVerse} disabled={currentVerse >= currentSurah.numberOfAyahs} className="w-8 h-8 sm:w-9 sm:h-9">
                  <SkipForward className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                </Button>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between text-xs sm:text-sm mb-1 gap-2">
                  <span className="text-foreground font-medium truncate">Verse {currentVerse}/{currentSurah.numberOfAyahs}</span>
                  <span className="text-muted-foreground shrink-0">{formatTime(progress)}/{formatTime(duration || 0)}</span>
                </div>
                <div className="h-1 sm:h-1.5 bg-secondary rounded-full overflow-hidden">
                  <div className="h-full bg-primary transition-all duration-300" style={{ width: `${duration ? (progress / duration) * 100 : 0}%` }} />
                </div>
              </div>
        ...
        <div className="flex items-center justify-between mb-6 animate-slide-up">
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "reading" | "arabic" | "tafsir")}>
            <TabsList>
              <TabsTrigger value="reading" className="gap-1.5 text-xs sm:text-sm">
                <BookOpen className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                Reading
              </TabsTrigger>
              <TabsTrigger value="arabic" className="gap-1.5 text-xs sm:text-sm">
                <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                Arabic
              </TabsTrigger>
              <TabsTrigger value="tafsir" className="gap-1.5 text-xs sm:text-sm">
          ...
          <Card variant="subtle" className="mb-6 animate-fade-in">
            <CardContent className="p-4 space-y-4">
              <div className="grid gap-4 sm:grid-cols-3">
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">Translation</label>
                ...
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">Reciter</label>
                  <Select value={selectedReciter} onValueChange={setSelectedReciter}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {RECITERS.map((r) => (
                        <SelectItem key={r.id} value={r.audioIdentifier}>{r.name}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">Tafsir Source</label>
        ...
        {error && (
          <Card variant="subtle" className="text-center py-12">
            <CardContent>
              ...
              <Button variant="subtle" onClick={() => window.location.reload()}>Retry</Button>
            </CardContent>
          </Card>
        )}

        {/* Reading Tab - Verses with Translation */}
                ...
                <CardContent className="p-6">
                  <div className="flex items-start gap-4 mb-4">
                    <div className={cn(
                      ...
                      currentVerse === verse.numberInSurah && isPlaying ? "bg-primary text-primary-foreground" : "bg-primary/10"
                    )}>
                      <span className={cn("text-sm font-medium", currentVerse === verse.numberInSurah && isPlaying ? "text-primary-foreground" : "text-primary")}>
                        {verse.numberInSurah}
                      </span>
                    </div>
                    <div className="flex-1">
                      <p className="arabic-lg text-foreground text-right leading-loose mb-4">{verse.arabic}</p>
                      <p className="text-foreground leading-relaxed">{verse.translation}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-4 sm:p-6">
                  <div className="flex items-start gap-3">
                    <div className={cn(
                      ...
                      <span className={cn("text-xs font-medium", currentVerse === verse.numberInSurah && isPlaying ? "text-primary-foreground" : "text-primary")}>
                        {verse.numberInSurah}
                      </span>
                    </div>
                    <p className="arabic-lg text-foreground text-right leading-[2.5] flex-1">
                      {verse.arabic}
                    </p>
                  </div>
                </CardContent>
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start gap-4">
                        <div className="w-8 h-8 rounded-full bg-gold/20 flex items-center justify-center shrink-0">
                          <span className="text-sm font-medium text-gold">{item.verseNumber}</span>
                        </div>
                        <div className="flex-1">
                          <p className="text-foreground leading-relaxed whitespace-pre-wrap">{item.text}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
        ...
        <div className="flex items-center justify-between mt-8 py-6 border-t border-border animate-slide-up" style={{ animationDelay: '0.2s' }}>
          {prevSurah ? (
            <Button variant="subtle" onClick={() => navigate(`/quran/${prevSurah.number}`)}>
          ...
          {nextSurah ? (
            <Button variant="subtle" onClick={() => navigate(`/quran/${nextSurah.number}`)}>
